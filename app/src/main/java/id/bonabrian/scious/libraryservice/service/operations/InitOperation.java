...
public class InitOperation extends AbsBTLEOperation<MiBand2Support> {
    ...
    protected void doPerform() throws IOException {
        ...
        if (needsAuth) {
            builder.add(new SetDeviceStateAction(getDevice(), SciousDevice.State.AUTHENTICATING, getContext()));
            ...
            builder.add(new SetDeviceStateAction(getDevice(), SciousDevice.State.INITIALIZING, getContext()));
    ...
    public boolean onCharacteristicChanged(BluetoothGatt gatt, BluetoothGattCharacteristic characteristic) {
        ...
        if (MiBand2Service.UUID_CHARACTERISTIC_AUTH.equals(characteristicUUID)) {
            try {
                ...
                Toast.makeText(getContext(), "Error authenticating Mi Band 2", Toast.LENGTH_LONG).show();
            }
            return true;
        } else {
            Log.i("InitOperation", "Unhandled characteristic changed: " + characteristicUUID);
            return super.onCharacteristicChanged(gatt, characteristic);
        }
    }
